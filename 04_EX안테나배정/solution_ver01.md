```cpp
/*
30_EX안테나배정_user_ver01_code

[해결 전략 1]
1. 단말기에 우선순위를 두어 안테나를 배정하기
   (1) 각 단말기별로 모든 안테나까지 거리를 구하고
       안테나들을 거리의 오름차순으로 정렬한다.
   (2) 단말기 아이디를 단말기와 가장 가까운 TH개 안테나 까지 거리 제곱의 합의 오름차순으로 정렬한다.
   (3) 가장 가까운 TH안테나까지 거리제곱의 합이 가장 먼 단말기를 우선으로 안테나를 배정한다.
...
4. 스케치 코드에서는 단말기가 4번 이동하는 동안 안테나의 가동범위를 벗어날 수 있으므로
   ...
   (2) 두번째 부분문제에서 각 단말기의 방향을 구한다.
       ...
       두번째 ~ 10번째 부분문제에서는 모든 안테나에 일괄적으로 +4 하는 일을 하지 않으므로
       약 100만여 점을 절약하게 된다.

[고찰]
  점수는 안테나 가동범위 제곱의 합으로 계산된다.
  점수를 높이기 위하여 단말기 입장에서 가장 가까운 안테나를 선택하는 것이 최선일까?
  단말기가 안테나를 선택할 때, 점수를 높이기 위한 다른 전략은 무엇이 있을까?

SEED: 5   SCORE: 2'050'331
PASS
*/
...
void process1() {
	...
	for (int i = ULM - 1; i >= 0; --i) {
		...
		for (int j = 0; j < ALM; ++j) {
			aid = tab[uid][j].id, minDist = tab[uid][j].cost;
			if (antUeCnt[aid] < 100) break; // tid안테나의 수용여력이 있다면 안테나를 선택
		}
...
void scanUE(Coordinates UE_list[], int antenna_range[], int antenna_UE_connection[]) {
	...
	// implement here
	for (int i = 0; i < ULM; ++i) {
		int uid = UE[ULM - i - 1].id, aid, minDist;
		for (int j = 0; j < ALM; ++j) {
			aid = tab[uid][j].id, minDist = tab[uid][j].cost;
			if (antUeCnt[aid] < 100) break; // tid안테나의 수용여력이 있다면 안테나를 선택
		}
