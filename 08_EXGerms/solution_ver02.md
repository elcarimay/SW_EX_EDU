...
5. 탐색대상을 조금 더 축소하는 방법을 생각해 볼 수 있다.
   tgColor와 같은 일반세균중에서 1st_match()를 수행한 결과에서 
   sim1 > 180+크기차를 벗어나는 일반세균을 rotate() + 2nd_match() 할 필요가 없다.
   이들을 제외하고 rotate() + 2nd_match() 수행하면 아래와 같은 점수향상을 얻을 수 있다.
...
void process()
{
	...
	for (int i = id; i < GLM; ++i) {
		if (core >= CLM) core = 0;
		if (tgColor != grr[i].color) continue; // 색이 다른 경우는 제외하고
		match(core, i);
		blen[core] += 3;
		++core;
	}
	...
	for (int i = 0; i < CLM; ++i) {
		for (int j = 0; j < blen[i]; j += 3) {
			int gid = buf[i][j + 1];
			int sim1 = buf[i][j + 2];
			if (sim1 > 180 + Abs(tgSize - grr[gid].size))
				grr[gid].color = -1;    // 탐색할 가치가 없는 세균은 제외하기 위하여
			else grr[gid].sim = sim1;
		}
	}
	...
	for (int i = 0; i < CLM; ++i) {
		for (int j = 0; j < blen[i]; j += 3) {
			int gid = buf[i][j + 1];
			grr[gid].sim += buf[i][j + 2];
			if (grr[gid].sim == 180 + Abs(tgSize - grr[gid].size) * 2)
				ans++;
		}
	}
