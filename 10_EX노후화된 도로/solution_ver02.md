...
5. 각 간선의 사용상황을 시뮬레이션한 결과를 먼저 구한다.             // ##### 수정된 부분
   수선이 필요한 간선은 빈 트럭으로 운송시에 최대로 수선한다.
   수선비용 계산식은 (10 + 수선값*(화물무게+10))임을 이용한다.

TC: 1,  SEED: 1, SCORE:  11'179'775 =>  11'177'414 (빈차일때 최대로 수선하기)
PASS

TC: 10, SEED: 1, SCORE: 111'184'946 => 111'149'715 (빈차일때 최대로 수선하기)
PASS
	...
	for (int i = 1; i <= cmdLen; ++i) {
		if (cmds[i].cmd) {
			...
			if (minusStatus[rid] < 0) {
				if (totWei == 10) {   // 빈차이고 rid간선에 수선이 필요한 경우
