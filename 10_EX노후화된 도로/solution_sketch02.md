...
TC: 1,  SEED: 1, SCORE:  11'178'852 =>  11'175'552 (출발지와 도착지가 같은 경우는 제외한 점수)
PASS

TC: 10, SEED: 1, SCORE: 111'291'198 => 111'196'753 (출발지와 도착지가 같은 경우는 제외한 점수)
FAIL

[고찰]
dist[]를 단순히 점수(비용)만으로 업데이트 하고 있다.
이를 Data타입으로 하여 비교한다면 위에서 수정한 우선순위가 적용된다.
*/
struct Road
{
	int city1;
...
struct Data {
	...
	bool operator<(const Data&t)const { 
		if(cost != t.cost) 
			return cost < t.cost;
		return statSum > t.statSum;       // ##### 수정된 부분
	}
};
int dist[LM];                             // *** 개선이 필요
	...
	while (!pq.empty()) {
		...
		for (int i = 0; i < alen[t.city]; ++i) {
			...
			if (dist[nxt.city] > nCost) {
				...
				pq.push({ nxt.city, nCost, t.statSum + status[nxt.rid] + repair - totWei });  // ##### 바뀐부분
			}
		}
	}
