```cpp
/*
47_EX우주선착륙_user_ver04

[해결전략 4]
* [해결전략 3]에서 각 루트 그룹별로 멤버들을  score 의 내림차순으로 정렬후 최적조합 찾기
				   <ver01>    <ver02>    <ver03>    <ver04>
seed :   5  SCORE: 3001673(최적해)
seed :  32  SCORE: 3011024 => 3011261 => 3011679 => 3011789
seed : 176  SCORE: 3022040 => 3022226 => 3022447 => 3022616
...
// #### ver4에서 추가된 부분 : 각 그룹의 크기가 크지 않으므로 insertionSort로 해결한다.  #####
void insertionSort(int grr[], int len) {  // 착륙지 아이디를 착륙지 점수의 내림차순으로 정렬한다.
	int i, j;
	for (i = 1; i < len; ++i) {
		int t = grr[i];
		for (j = i - 1; j >= 0; --j) {
			if (lands[grr[j]].score < lands[t].score ) grr[j + 1] = grr[j];
			else break;
		}
		grr[j + 1] = t;
	}
